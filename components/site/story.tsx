...
function StatCounter({
  ...
  return (
    <div className="flex flex-col items-center gap-1 rounded-2xl border border-border bg-card/50 px-4 py-5 text-center">
      <span
        className="font-heading text-3xl leading-none font-bold md:text-4xl"
        style={{ color }}
      >
    ...
    stats: [
      {
        value: 40,
        suffix: "%",
        label: "less downtime after migration",
        color: "#a78bfa",
      },
      {
        value: 30,
        suffix: "%",
        label: "faster deployments with CI/CD",
        color: "#a78bfa",
      },
      {
        value: 25,
        suffix: "%",
        label: "productivity gained via mentoring",
        color: "#a78bfa",
      },
      {
        value: 12,
        suffix: "",
        label: "engineers led as manager",
        color: "#a78bfa",
      },
    ],
    showcase: <CircuitBoard />,
    ...
    stats: [
      {
        value: 94,
        suffix: "%",
        label: "sprint delivery (was 86%)",
        color: "#34d399",
      },
      {
        value: 7.8,
        suffix: "",
        label: "avg velocity / sprint (was 6.5)",
        color: "#34d399",
      },
...
export function Story() {
  return (
    <section id="story" className="mx-auto max-w-250 w-4/5 px-4 py-24 md:px-8">
      <div className="flex flex-col gap-28">
        {chapters.map((chapter, i) => (
          <div key={chapter.id} className="relative">
            <AnimatedContent distance={60} duration={0.7} threshold={0.15}>
              <div className="grid grid-cols-1 items-center gap-8 md:grid-cols-5 md:gap-6">
                <div className={cn("min-w-0 md:col-span-3", i % 2 === 1 && "md:order-2")}>
                  <div
                    className="col-span-4 mb-2 inline-flex items-center gap-2 font-mono text-xs tracking-[0.25em] uppercase"
                ...
                <div className={cn("min-w-0 md:col-span-2", i % 2 === 1 && "md:order-1")}>{chapter.showcase}</div>
              </div>
      ...
      <AnimatedContent distance={40} threshold={0.3}>
        <Card className="mt-24 border-primary/30 bg-primary/5 p-6 md:p-8">
          <p className="font-heading text-lg leading-relaxed md:text-xl">
            There&apos;s a version of my story that isn&apos;t in the résumé:
            the open-source packages with steady weekly downloads, the merged
            NestJS docs PR, the president-of-the-student-association era, and
            the food adventures I mention whenever someone asks about Honduras.{" "}
            <span className="text-primary">Keep scrolling</span> — or jump to
            the{" "}
            <a
